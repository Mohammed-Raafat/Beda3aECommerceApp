...
import { setFilteredProducts, setPriceFilter, setPrice } from '../store/actions';
import { productsFilteredByCategories, productsFilteredByPrice, getMinMax, isAllFalse } from './../OwnMethods';

const Filters = (props) => {
    const { products, price, filteredProducts, filteredCategories, filteredPrice, setPriceFilter, setFilteredProducts, setPrice } = props;
    ...
    return (
        <Grid style={{padding: 5}}>
            <CheckBoxList
                title='Categories'
                checkBoxList={filteredCategories}
            />
            <Divider />
            <RangeSlider
                title='Price ($)'
...
const mapDispatchToProps = (dispatch) => {
    return {
        setFilteredProducts: (newFilteredProducts) => dispatch(setFilteredProducts(newFilteredProducts)),
        setPriceFilter: (newFilteredPrice) => dispatch(setPriceFilter(newFilteredPrice)),
